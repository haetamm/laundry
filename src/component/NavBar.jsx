    ...
    <>
      <div className={`nav-bar-container ${showNavBar ? "visible" : "hidden"}`}>
        <div
          className={`${
            token
              ? "ml-0 xl:w-[calc(100%-60px)] xl:ml-[60px] bg-black opacity-95 px-[15px] py-[5px]"
              : "ml-0 bg-white shadow-md"
          } text-sm font-semibold wrap-navbar flex justify-between`}
        >
          <div className="flex space-x-3 items-center">
            <div className="item-center space-x-1 items-center px-2 py-1.5 text-lg cursor-pointer">
              {token && (
                <div className="flex tab:hidden ml-0 xs:ml-[60px] xl:ml-0">
                  {path === "user" ? "ADMIN" : path.toUpperCase()}
                </div>
              )}
              {!token && (
